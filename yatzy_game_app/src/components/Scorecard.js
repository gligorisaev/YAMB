...
const Scorecard = ({ player, scores, updateScores, editable }) => {
    ...
    columns.forEach((col) => {
        ...
        // Aggregate R column sums
        sum1to6["R"] = ["↓", "↓↑", "↑", "N", "O"]
            .map((col) => sum1to6[col] || 0)
            .reduce((acc, val) => acc + val, 0);
        sumMaxMin["R"] = ["↓", "↓↑", "↑", "N", "O"]
            .map((col) => sumMaxMin[col] || 0)
            .reduce((acc, val) => acc + val, 0);
        sumT20Y60["R"] = ["↓", "↓↑", "↑", "N", "O"]
            .map((col) => sumT20Y60[col] || 0)
            .reduce((acc, val) => acc + val, 0);
    });
    ...
    const toggleHighlight = () => {
        if (highlightedCell) {
            ...
            const newScores = {
                ...
                [category]: {
                    ...scores[category],
                    [`${col}_highlighted`]: !scores[category]?.[`${col}_highlighted`],
                },
            };
            updateScores(newScores);
        }
